...
root_dir = 'D:\\Downloads\\cholec80\\'
img_dir = os.path.join(root_dir, 'data_resize')
tool_dir = os.path.join(root_dir, 'tool_annotations_temp')
phase_dir = os.path.join(root_dir, 'phase_annotations_temp')
...
for j in range(len(tool_file_names)):
    ...
    for tool_line in tool_file:
        ...
        if tool_count > 1:
            ...
            info_each = []
            if tool_count <= 10:
                img_file_each_path = os.path.join(img_dir_paths[j], "output" + '_00' + str(tool_count - 1) + '.png')
            elif tool_count > 10 and tool_count <= 100:
                img_file_each_path = os.path.join(img_dir_paths[j], "output" + '_0' + str(tool_count - 1) + '.png')
            else:
                img_file_each_path = os.path.join(img_dir_paths[j], "output" + '_' + str(tool_count - 1) + '.png')
