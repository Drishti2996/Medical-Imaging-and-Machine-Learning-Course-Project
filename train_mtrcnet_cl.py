...
# Adapted the code from Jin, Yueming and Li, Huaxia and Dou, Qi and Chen, Hao and Qin, Jing and Fu, Chi-Wing and Heng, Pheng-Ann (2020) https://github.com/YuemingJin/MTRCNet-CL.git

num_gpu2 = torch.cuda.device_count()
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    ...
    model_old.load_state_dict(torch.load(
        "cnn_lstm_epoch_25_length_4_opt_1_mulopt_1_flip_0_crop_1_batch_100_train1_9989_train2_9970_val1_9543_val2_8197.pth"))
    ...
    for epoch in range(epochs):
        ...
        for i, data in  enumerate(tqdm(train_loader)):
            ...
            preds_1 = torch.BoolTensor(sig_output_1.data.cpu() > 0.5)
            preds_1 = preds_1.long()
            train_corrects_1 += torch.sum(preds_1 == labels_1.data.cpu())

            preds_3 = torch.BoolTensor(sig_average.cpu() > 0.5)
            preds_3 = preds_3.long()
            train_corrects_3 += torch.sum(preds_3 == labels_1.data.cpu())
            ...
            train_loss_2 += loss_2.data
            train_loss_3 += loss_3.data
            print("time = " + str(time))
            tqdm.write(f"Epoch {epoch+1}/{25}, Batch {i+1}/{len(train_loader)}, Loss: {loss:.4f}")
        ...
        for data in val_loader:
            ...
            preds_3 = torch.BoolTensor(sig_average.cpu() > 0.5)
            preds_3 = preds_3.long()
            val_corrects_3 += torch.sum(preds_3 == labels_1.data.cpu())
            ...
            val_loss_1 += loss_1.data
            val_loss_2 += loss_2.data
            val_loss_3 += loss_3.data
